...
export default function Racks() {
  ...
  const { data: racks, isLoading } = useListRacks(
    { roomId },
    { query: { queryKey: getListRacksQueryKey({ roomId }), enabled: !!roomId } }
  );
  ...
  const handleCreate = (e: React.FormEvent) => {
    e.preventDefault();
    createRack.mutate(
      { data: { name, totalU: parseInt(totalU, 10), description, roomId } },
      {
        onSuccess: () => {
          setIsOpen(false);
  ...
  return (
    <div className="space-y-6">
                ...
                <div className="grid grid-cols-4 gap-4">
                  <div className="space-y-2 col-span-3">
                    ...
                    <Input
                      ...
                      placeholder="e.g. R01-A"
                    />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="totalU" className="text-xs uppercase tracking-wider text-muted-foreground">
                      Height (U)
                    </Label>
                    <Input
                      id="totalU"
                      ...
                      max="52"
                      value={totalU}
                      onChange={(e) => setTotalU(e.target.value)}
                      required
                      className="font-mono bg-background"
                    />
                  </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {isLoading ? (
          <div className="col-span-full h-24 flex items-center justify-center font-mono text-xs text-muted-foreground uppercase tracking-widest">
            Loading…
          </div>
        ) : !racks?.length ? (
          <div className="col-span-full h-24 flex items-center justify-center font-mono text-xs text-muted-foreground uppercase tracking-widest border border-border border-dashed rounded-md">
            No racks provisioned in this room
          </div>
        ) : (
          racks.map((rack) => {
            ...
            return (
              <div
                key={rack.id}
                className="border border-border rounded-md bg-card/50 overflow-hidden flex flex-col group hover:border-primary/50 transition-colors"
              >
                <div className="p-4 border-b border-border flex justify-between items-start">
                  <div>
                    ...
                    {rack.description && (
                      <p className="text-xs text-muted-foreground mt-1 line-clamp-1">{rack.description}</p>
                    )}
                  </div>
                  {canWrite && (
                    <Button
                ...
                <div className="p-3 bg-secondary/30 border-t border-border mt-auto">
                  <Link
                    href={`/racks/${rack.id}/view`}
                    className="w-full flex items-center justify-center gap-2 text-xs font-mono uppercase tracking-widest text-primary hover:text-primary-foreground hover:bg-primary py-2 rounded transition-colors"
                  >
