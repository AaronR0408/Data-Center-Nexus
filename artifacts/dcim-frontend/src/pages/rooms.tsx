...
export default function Rooms() {
  ...
  const { data: site } = useGetSite(siteId, {
    query: { queryKey: getGetSiteQueryKey(siteId), enabled: !!siteId },
  });
  const { data: rooms, isLoading } = useListRooms(
    { siteId },
    { query: { queryKey: getListRoomsQueryKey({ siteId }), enabled: !!siteId } }
  );
          ...
          <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="sm:max-w-[425px] bg-card border-border">
              ...
              <form onSubmit={handleCreate} className="space-y-4 pt-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="floor" className="text-xs uppercase tracking-wider text-muted-foreground">
      ...
      <div className="border border-border rounded-md bg-card/50 overflow-hidden">
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              <TableRow>
                <TableCell colSpan={4} className="h-24 text-center font-mono text-xs text-muted-foreground uppercase tracking-widest">
                  Loading…
                </TableCell>
              </TableRow>
            ) : !rooms?.length ? (
              <TableRow>
                <TableCell colSpan={4} className="h-24 text-center font-mono text-xs text-muted-foreground uppercase tracking-widest">
                  No rooms provisioned in this site
                </TableCell>
              </TableRow>
            ) : (
              rooms.map((room) => (
                <TableRow key={room.id} className="border-border transition-colors hover:bg-secondary/20">
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-2">
                      <Link
                        href={`/rooms/${room.id}/racks`}
                        className="inline-flex items-center justify-center rounded-md text-xs font-mono font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring border border-border bg-background hover:bg-accent hover:text-accent-foreground h-8 px-3 py-2"
                      >
